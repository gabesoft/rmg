...
module.exports = function () {
  var app  = this,
      ...
      src  = path.join(app.root, 'templates/view.js'),
      dst  = path.join(app.module.path, app.config.get('paths').views, name.path),
      opts = {
        module : camelize(app.argv['module-name'] || app.module.name),
        name   : camelize('-' + name.base),
        type   : app.argv.type || 'ItemView'
      };

  if (name.base.length === 0) {
    return cb(new Error('No view name specified'));
  }
