...
app.use(flatiron.plugins.cli, {
  ...
  usage: [
    ...
    '- generates a dust template file in the templates folder',
    '- params: --name <file-name>' + '        file name without extension'.grey,
    '-         --add-to-config [key]' + '     add the file to the build config under the'.grey,
    '-                                   given key'.grey + ' (' + 'defaults to module name, alias --key'.grey + ')',
    '',
    ...
    '-         --module-name <name>' + '      optional marionette module name'.grey + ' (' + 'alias --mn'.grey + ')',
    '',
    ...
    '- params: --module-name <name>' + '      optional marionette module name'.grey + ' (' + 'alias --mn'.grey + ')',
    '',

    app.name.yellow + ' version'.yellow + '               [' + 'alias: '.grey + app.name.grey + ' ver'.grey + ']',
...
app.use(require('./plugins/config'));
...
app.start(function (err, cfg) {
  if (err && err !== true && err !== false) {
    ...
    if (app.module && app.module.save) {
      cfg['module-data'] = {
        relpath: app.module.relpath,
        objname: app.module.objname
      };
    }
    ...
    Object.keys(cfg).forEach(function (name) {
      ...
      app.config.save();
      log.data('config updated ' + name.yellow + ' = ' + JSON.stringify(cfg[name]).magenta);
    });
  }
});
